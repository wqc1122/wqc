...
def search_button():
	...
	try:
		...
		def ShowInfo():
			global InputDate, Pred, Act, Acc, InputDateEntry, pred, act, acc, Pred_button
		...
		def pre_plot():
			...
			t = np.array([datetime.strptime(str(tt), '%Y-%m-%d') for tt in test_date_group])
		...
		tk.messagebox.showerror("Error","Symbol Not Found2")
